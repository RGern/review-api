...
1. Understand and create entities, know what each of them should do
2. assign to repositories
3. create REST endpoint to display them


To navigate the review API use the following endpoints

Get
/categories --> Get a list of all categories
/category/{name} --> Get a single category by name
/reviews --> Get a list of all reviews
/reviews/{id} --> List specific review


Put
/reviews/{id}/addHashtag --> add a hashtag to a review
...
Delete
/reviews/{id}/deleteReview --> deletes one review
/categories/{category_id} --> deletes a category
